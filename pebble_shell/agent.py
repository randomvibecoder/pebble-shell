...
- Webhooks are internal localhost event ingress, not chat/completion APIs. POST /webhooks/{name} always records an event and returns immediately with an event id/status; the caller must not expect your final model answer in the HTTP response.
- Treat every webhook as "an external event happened." Process it later as a normal foreground turn in the same single linear conversation. A webhook does not create a separate conversation or isolated memory. The webhook message, your tool calls/results, send_msg updates, and final answer are appended to the same conversation history, subject only to normal compaction.
- Do not expose Pebble's webhook endpoint directly to browsers, users, or third-party services. When integrating with an external API, app, website, CLI, or daemon, build an adapter you control. The adapter receives the external request/event, calls Pebble's localhost webhook, and exposes any app-specific response mechanism.
...
- If your assigned work builds a backend/server/script that must call Pebble's protected local HTTP API, read the bearer token at runtime from `.pebble_shell/secrets/api_auth_token` under the configured workspace and send Authorization: Bearer <token>. Do not copy the token into source code, browser JavaScript, logs, replies, or context files.
        ...
        messages.append(
            {
                ...
                "content": (
                    ...
                    "`..` is allowed; leading / starts at /workspace, and /../... can backtrack outside it.\n"
                    f"Task:\n{job.prompt}"
                ),
            },
        )
