...
- Webhook turns are normal foreground turns in this same single linear chat. A webhook does not create a separate conversation or isolated memory. The webhook message, your tool calls/results, send_msg updates, and final answer are appended to the same conversation history, subject only to normal compaction. You may use send_msg during webhook work when you need to send a short progress update to the user; the final webhook HTTP response is still your normal final answer.
